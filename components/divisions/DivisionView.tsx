...
/** Scroll-scrubbed hero frame sequences, keyed by division slug. Generated by
 * scripts/extract_hero_frames.py — `count` must match the number of files it
 * wrote (the script prints it). A slug with no entry falls back to the
 * division's static `image`.
 *
 * `tone` is the hero copy's ink, chosen from the opening frames' measured
 * luminance. Both clips currently open on a white frame (255/255) so both use
 * dark copy; the prop stays because it is a property of the footage, not the
 * design — swap in a clip that opens dark and this has to flip to "light". */
const HERO_FRAMES: Record<
  string,
  { dir: string; count: number; tone: "light" | "dark" }
> = {
  building: { dir: "/division-frames/building", count: 274, tone: "dark" },
  "oil-and-gas": { dir: "/division-frames/oil-and-gas", count: 271, tone: "dark" },
};
...
export function DivisionView({
  ...
  const frames = HERO_FRAMES[division.slug];
